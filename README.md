...
- **거래처 자동 등록** — 신규 거래 입력 시 거래처명 입력만으로 자동 등록. 별도 등록 불필요
- **상품 자동 등록** — 신규 거래 상품 항목에 상품명 입력 시 자동 등록. 컬러·단가 자동완성
...
- **구글 드라이브 백업** — DB 파일 전체를 드라이브에 백업/복원
- **텔레그램 봇** — `/today /month /unpaid /due /stock /tax` 명령어로 외부에서 조회
- **자동 업데이트** — 새 버전 출시 시 앱 실행 중 업데이트 알림 및 자동 설치
...
- ✅ **v0.2.5** — 거래 폼에서 거래처·상품 자동 등록, 구글 드라이브 DB 백업/복원
- ✅ **v0.2.6** — 지불상태 "대납" 표기, 거래 삭제(재고 자동 복원), 상품 삭제(단종 처리), 상품 검색, 미송 저장 버그 수정
- ✅ **v0.2.7** — 거래처·상품 소프트 삭제(거래내역 이름 유지), 거래처 삭제 기능, 대시보드 미송 현황 버그 수정
