                ...
                <div className="rounded-md bg-neutral-50 px-3 py-2 text-sm">
                  ...
                  <span
                    className={`font-semibold ${
                      avgSales - avgExpense >= 0 ? "text-emerald-700" : "text-red-700"
                    }`}
                  >
                    {formatKRW(avgSales - avgExpense)}
                  </span>
                  <span className="ml-2 text-xs text-neutral-400">
                    (최근 {recent.length}개월 평균)
                  </span>
                </div>
              </div>
            )}
            ...
            {lowStock.map((p) => (
              <li key={p.id} className="flex items-center justify-between py-2 text-sm">
                <div className="flex items-center gap-1.5">
                  ...
                  {p.color && (
                    <span className="text-xs text-neutral-500">{p.color}</span>
                  )}
                </div>
                <span
                  className={`font-semibold ${
        ...
        >
          ...
          <div className="mt-3 flex items-center justify-between text-sm">
            <span className="text-neutral-600">
              신고 기한: {taxDeadline.deadlineDate}
            </span>
            <span className="font-medium text-neutral-900">
              예상 납부세액 {formatKRW(taxDeadline.estimatedVat)}
            </span>
          </div>
        </div>
      )}
